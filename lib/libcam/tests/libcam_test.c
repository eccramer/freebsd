...
ATF_TC(libcam_get_device_invalid_device);

ATF_TC_HEAD(libcam_get_device_invalid_device, tc)
{

	atf_tc_set_md_var(tc, "descr", "test cam_get_device with a invalid device name.");
}

ATF_TC_BODY(libcam_get_device_invalid_device, tc)
{
	char* path = "/dev/foo1";
	char  dev_name[10];
	int   devnamelen = 10;
...
ATF_TP_ADD_TCS(tp)
{
	...
	ATF_TP_ADD_TC(tp, libcam_get_device_invalid_device);
